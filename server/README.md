...
journal, and refused within ONE `-scopes` list as well. A name that already exists as a
directory under `-store` is **warned about, not
...
In one real deployment it turned out to be the node's **`cilium_host` router
address** — and none of the three obvious candidates were right:

| candidate | what it actually is | right? |
|---|---|---|
| the gateway's **mesh/VPN** IP | what the gateway's proxy *listens* on — not what it connects FROM | ✗ |
| the node's Kubernetes **InternalIP** | the node's address on the LAN, not the address seen inside the pod | ✗ |
| the node's **`cilium_host` router address** | what the gateway's traffic is SNAT'd to on the way into the pod | ✓ |
