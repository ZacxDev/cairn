...
------------------------------------------------------------------------
Exposure **B** is a public hostname behind a CDN/reverse proxy, where `/api/*`
carries NO edge auth — a redirect-based SSO is unusable from a CLI. So the
bearer token is the ONLY thing between the public internet and confidential
content, and §2b's hardening stops being optional:
