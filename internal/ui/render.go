...
// headerClasses is the page banner's layout, spelled ONCE because all three pages render
// the same banner and a fourth spelling is how two of them drift apart.
//
// ⚠ THESE ARE TAILWIND UTILITIES IN A GO STRING, AND THAT IS WHAT THE GENERATOR READS.
// `tailwind.css` declares `@source "./*.go"`, so the class names below are extracted from
// THIS FILE — a utility written here and nowhere else still ends up in `app.css`. The
// converse is the hazard worth naming: a class name assembled at run time from pieces
// (`"text-" + size`) is invisible to that scan and would silently not exist in the
// stylesheet, so every class on this surface is a literal.
const headerClasses = "flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1 " +
	"border-b border-line pb-4 mb-6"
...
func SignInPage(message string, provider bool) g.Node {
	return c.HTML5(c.HTML5Props{
		...
		Head:     []g.Node{stylesheetLink()},
		Body: []g.Node{
			h.Header(h.Class(headerClasses), h.H1(g.Text("cairn"))),
			h.Main(
				h.Class("bg-raised border border-line rounded-card px-5 py-6 max-w-md animate-rise"),
