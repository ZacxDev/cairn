...
| `cmd/cairn-ui`, `internal/ui` | the BROWSER surface — BROWSE+SEARCH, gomponents, COOKIE SESSIONS, the SHARE FLOW, GitHub sign-in; **DEPLOYED**; 📄 its own README |
...
🔴 **ENTRY STRUCTURE COMES FROM `internal/store`'s PARSERS — NEVER `internal/report`, NEVER
A NEW MARKDOWN READER.** README.
