...
which.** Credentials, private IPs, dated incidents AND hosts under the deployment's
registrable domains (`reachable-hostname`, an UNBOUNDED regex) are gated in GENERAL. Gated only
for a CLOSED SET of digests is the REST of the project/scope/host class (`denied-identifier`):
a name a scrub removed cannot return, a NEW one is invisible until you add it. Captured text
is gated by NOTHING and cannot be; that bullet is yours. ⚠ This line has been wrong BOTH ways —
all five over three, then omitting `reachable-hostname`; why neither is a heuristic is
`tests/leakscan.py`'s docstring. Run it before you push:
...
| `cmd/cairn-ui`, `internal/ui` | the BROWSER surface — pages, gomponents, COOKIE SESSIONS, the SHARE FLOW, GitHub sign-in; **DEPLOYED** (the retraction is in its README) |
