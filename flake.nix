  ...
  outputs = { self, nixpkgs }:
    let
      ...
      onlyGo = pkgs: pkgs.lib.cleanSourceWith {
        ...
        filter = path: type:
          ...
          # COMPILE, so every Go derivation here would go red at once. Named explicitly
          # for the reason the `.go` row below gives — a `.css` suffix rule would be an
          # allowlist that says something wider than it means.
          #
          # ⚠ `tailwind.css` IS DELIBERATELY *NOT* HERE, AND AN EARLIER DRAFT CARRIED IT.
          # That draft's `uiStylesheet` built from this filtered tree because the generator
          # SCANNED the `.go` files — so the sandbox needed both the source and everything
          # the scan read. The scan was removed (see `internal/ui/tailwind.css` for the
          # measurement that forced it), the generator now takes exactly one file, and that
      ...
      # 🔴 THE BROWSER SURFACE'S STYLESHEET IS BUILD OUTPUT, AND THIS IS THE BUILD.
      # `internal/ui/tailwind.css` is the source; `internal/ui/app.css` is what
      # `internal/ui/stylesheet.go` embeds and what `handleStylesheet` serves. The two
      # are kept in step by `checks.ui-stylesheet-is-current`, which runs this and
      # compares — so the discipline is the one `reader_fixtures.json` carries:
      # REGENERATE AND DIFF, never hand-edit the output.
      #
      # ⚠ THE TAILWIND VERSION IS WHATEVER THE PINNED `nixpkgs` CARRIES, AND THAT IS A
      # DIMENSION THIS DERIVATION DOES NOT CONTROL. A `nixpkgs` bump that moves the CLI
      # will change the generated bytes and turn the check red until somebody
      # regenerates — which is the honest failure (the tree really is stale) rather
      # than a spurious one, and it is the same trade the pinned interpreter and the
      # pinned Go toolchain make. What it is NOT is a reason to relax the check into a
      # substring match: a comparison that tolerates a different Tailwind would tolerate
      # a hand-edit too, which is the whole thing it exists to refuse.
      #
      # 🔴 ITS INPUT IS EXACTLY ONE FILE, AND THAT IS THE REPRODUCIBILITY ARGUMENT RATHER
      # THAN A TIDINESS ONE. `tailwind.css` carries `source(none)` and declares no
      # `@source`, so nothing is scanned and the output is a pure function of these bytes
      # plus the pinned CLI. Whatever else is in the tree — a developer's full checkout, a
      # filtered sandbox source, a stray file — cannot change the answer.
      #
      # ⚠ AN EARLIER DRAFT BUILT FROM `onlyGo` BECAUSE THE GENERATOR SCANNED THE `.go`
      # FILES, AND THE DRAFT'S OWN COMMENT ARGUED THAT WAS THE SAFE CHOICE. It was not:
      # Tailwind's extractor reads COMMENTS as readily as code, this package's comments are
      # dense by house rule, and six utilities were generated out of ordinary English with
      # no class literal anywhere. The consequence was a comment-only commit turning the
      # check below RED with a message blaming a hand-edit that never happened. The scan is
      # gone; the measurement is in `internal/ui/tailwind.css`.
      uiStylesheet = pkgs: pkgs.runCommand "cairn-ui-app.css"
      ...
      # the output has to be committed, and a store path cannot be.
      buildUIStylesheet = pkgs: pkgs.writeShellApplication {
        name = "build-ui-stylesheet";
        runtimeInputs = [ pkgs.tailwindcss_4 ];
        text = ''
          root="''${1:-$PWD}"
          in="$root/internal/ui/tailwind.css"
          out="$root/internal/ui/app.css"
          if [ ! -f "$in" ]; then
            echo "no $in — run this from the repository root, or pass the root as \$1" >&2
            exit 1
          fi
          tailwindcss -i "$in" -o "$out"
          echo "wrote $out"
        '';
      };
    ...
    in
    {
      ...
      apps = forAll (pkgs: {
        ...
        # ⚠ A DEVELOPER COMMAND, NOT A CLIENT, AND IT IS THE ONE APP HERE THAT WRITES TO
        # THE WORKING TREE. `nix run .#build-ui-stylesheet` regenerates
        # `internal/ui/app.css` from `internal/ui/tailwind.css`. It is not part of the
        # default/`cairn` wiring the check below pins, and adding it changes neither.
        build-ui-stylesheet = {
          type = "app";
      ...
      checks = forAll (pkgs: {
        ...
        # `tailwind.css` and forgetting to regenerate is loud: the build succeeds, every
        # test passes, and the surface serves the previous theme. This check regenerates
        # from the source inside the sandbox and refuses a difference.
        #
        # ⚠ IT PINS A RELATIONSHIP, WHICH IS WHY NEITHER SIDE ON ITS OWN WOULD DO. "The
        # CSS compiles" passes for a tree whose committed output is a month old; "the
        # committed file is non-empty" passes for one that was hand-written. The
        # comparison is the claim.
        #
        # 🔴 AND IT VALIDATES ITS OWN INSTRUMENT BEFORE READING ITS VERDICT. `diff`
        # returning 0 is indistinguishable from `diff` never running — a mistyped path, a
        # tool that is not on `PATH`, a comparison against a file that does not exist. So
        # a NEGATIVE CONTROL runs first: the same `diff`, over the same generated bytes
        # with one line appended, which MUST report a difference. If the control passes
        # quietly the check exits non-zero saying it could not vouch, rather than
        # reporting a green it has no basis for.
        ui-stylesheet-is-current =
          pkgs.runCommand "cairn-ui-stylesheet-is-current"
            { nativeBuildInputs = [ pkgs.diffutils ]; } ''
            generated=${uiStylesheet pkgs}
            committed=${./internal/ui/app.css}

            # Negative control FIRST. A `diff` that cannot go red makes the comparison
            # below a fact about the instrument and not about the tree.
            cp "$generated" ./control.css
            # A store path is read-only and `cp` carries the mode with it.
            chmod u+w ./control.css
            echo "/* a line no generator emits */" >> ./control.css
            if diff -q "$generated" ./control.css > /dev/null 2>&1; then
              echo "FAIL (could not vouch): the negative control COMPARED EQUAL. diff is"
              echo "not distinguishing files here, so the comparison below would report a"
              echo "green it has no basis for."
              exit 2
            fi

            if ! diff -u "$committed" "$generated"; then
              echo ""
              echo "FAIL: internal/ui/app.css is not what internal/ui/tailwind.css generates."
              echo "app.css is BUILD OUTPUT and must never be hand-edited. Regenerate it:"
              echo ""
              echo "    nix run .#build-ui-stylesheet"
              echo ""
