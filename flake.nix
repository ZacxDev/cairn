    ...
    let
      ...
      onlyGo = pkgs: pkgs.lib.cleanSourceWith {
        ...
        filter = path: type:
          ...
          # allowlist that says something wider than it means.
          #
          # `tailwind.css` is the SOURCE `app.css` is generated from. It is carried too,
          # not because the Go build needs it — it does not — but because
          # `checks.ui-stylesheet-is-current` regenerates from it inside the sandbox and
          # compares. A filter that carried only the output would make that check compare
          # the output against nothing.
          || (rel == "internal/ui/app.css")
          || (rel == "internal/ui/tailwind.css")
      ...
      #
      # 🔴 IT BUILDS FROM `onlyGo`, NOT FROM `./.`, BECAUSE THE SCAN IS PART OF THE
      # OUTPUT. `tailwind.css` declares `@source "./*.go"`, so the generated bytes depend
      # on which `.go` files are beside it — and a check that scanned the full tree while
      # a developer's `nix run .#build-ui-stylesheet` scanned the same directory would be
      # comparing two different questions the moment the filter and the tree disagree.
      # Both sides scan exactly `internal/ui/*.go`, which the filter carries in full.
      uiStylesheet = pkgs: pkgs.runCommand "cairn-ui-app.css"
        {
          nativeBuildInputs = [ pkgs.tailwindcss_4 ];
          src = onlyGo pkgs;
        } ''
        cp -r "$src/internal/ui" ./ui
        chmod -R u+w ./ui
        cd ./ui
