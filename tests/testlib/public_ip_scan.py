...
# 🔴 INLINED ON EXTRACTION, NOT IMPORTED. In the origin repository this regex
# came from a shared hook module so that two scanners could not disagree about
# what even LOOKS like an address. That module is a repository-hygiene tool and
# is not part of this project, and reaching across a `sys.path` insert into
# another tree is exactly the coupling extraction exists to remove — so the one
# symbol that was needed lives here instead.
#
# Deliberately permissive: it matches any dotted quad, INCLUDING out-of-range
# ones like `999.1.1.1`. Range-checking belongs to the caller that decides what
# is public, not to the candidate finder — a stricter pattern here would silently
# drop malformed addresses that a reader would still recognise as one.
