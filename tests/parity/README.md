...
| `packages.cairn`, `checks.client-resolves-its-lib`, and the `lib/` rule in `AGENTS.md` | the rule governs the client that is still shipped and still the oracle | ⚠ **HALF OF THIS ROW HAS ALREADY HAPPENED AND THE REMAINING HALF IS THE WHOLE CONDITION.** `packages.default`/`apps.default` cut over to the Go client in the image-and-default-cutover change; `packages.cairn` still exists, so the `lib/` rule still binds exactly as `AGENTS.md` says. The condition is now only: **`packages.cairn` is gone** |
