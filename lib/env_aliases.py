...
would read as a third member of the `CAIRN_CACHE_ROOT`/`CAIRN_MIRROR_ROOT` family
when it is the POD's store root and belongs to neither. Hence `CAIRN_STORE_ROOT`.
...
#: strings the cross-language gate has to pin and double the ways the two spellings
#: can drift, for a sentence that is already true either way as written: "read only
#: when … is unset" SAYS what happened when the new name is set.
#:
...
ENV_WARNING_FORMAT = (
    "${old} is a deprecated alias for ${new} and is read only when ${new} is unset. "
    "Both are accepted until {anchor}."
)
FILE_WARNING_FORMAT = (
    "{old} in {path} is a deprecated alias for {new} and is read only when {new} is "
    "absent from that file. Both are accepted until {anchor}."
)
...
def _present(mapping: Mapping[str, str]) -> list[tuple[str, str]]:
    ...
    return sorted(
        ((new, old) for new, old in LEDGER if mapping.get(old, "").strip()),
